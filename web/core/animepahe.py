...
from core.http_client import make_async_session, NETWORK_EXCEPTIONS
...
class AnimePaheClient:
   ...
   def _inject_cached_clearance(self, client: AsyncSession) -> None:
       ...
       # Pick the UA matching the AnimePahe host we actually talk to, rather
       # than letting the last stored host (possibly kwik) win, so the injected
       # cf_clearance and the request UA stay consistent (cf_clearance is
       # UA-bound).
       preferred = self.clearance_provider.get_compatible(host_from_url(self.base_url))
       if preferred and preferred.get("user_agent"):
           self.user_agent = str(preferred["user_agent"])
   ...
   def _learn_base_url_from_response(self, response) -> None:
       # Never override a user-pinned base URL (ANIMEPAHE_BASE_URL).
       if self._base_url_locked:
           return
       try:
           parsed = urlparse(str(response.url))
       except Exception:
           return
       if not (parsed.scheme and parsed.netloc and parsed.netloc.startswith("animepahe.")):
           return
       # Only adopt a (possibly redirected) host that actually served the JSON
       # API. This prevents a redirect to an HTML-only mirror (e.g. one that
       # returns 200 HTML instead of JSON) from poisoning the working base URL.
       try:
   ...
   def get_clearance_status(self) -> dict[str, object]:
       ...
       if self.clearance_provider:
           hosts = [host_from_url(self.base_url), "kwik.si"]
   ...
   async def close(self) -> None:
       if self._client is not None and not self._client_closed:
           ...
           # Force the next _get_client() to re-warm the recreated session.
           self._session_ready = False
   ...
   async def search(self, query: str) -> list[AnimeSearchResult]:
       ...
       try:
           ...
           raise
       except (*NETWORK_EXCEPTIONS, ValueError, KeyError, TypeError) as e:
           # Expected "couldn't reach/parse" cases map to an empty result set.
           # Unexpected exceptions are allowed to propagate so genuine bugs are
           # surfaced as errors instead of being masked as "no results".
   ...
   async def get_anime_details(self, session_id: str) -> dict:
       ...
       result = {
           "session": session_id,
           "title": title,
           "total_episodes": int(api_data.get("total") or 0),
           "last_page": int(api_data.get("last_page") or 1),
           "per_page": int(api_data.get("per_page") or 30),
           **mal_details,
   ...
   async def get_all_episodes(self, session_id: str) -> list[Episode]:
       ...
       # A single transient page failure must not abort the whole listing.
       # Collect the pages that succeeded and retry the failures once,
       # sequentially, before surfacing a clear error for a truly missing page.
       pages: dict[int, list[Episode]] = {}
       failed_pages: list[int] = []
       for page, result in zip(range(1, total_pages + 1), results):
       ...
       for page in failed_pages:
           try:
               pages[page] = await fetch_page(page)
           except Exception as exc:
               raise AnimePaheError(f"Failed to fetch episode page {page}: {exc}") from exc


       # Flatten and sort by episode number
       all_episodes = [ep for page in sorted(pages) for ep in pages[page]]
