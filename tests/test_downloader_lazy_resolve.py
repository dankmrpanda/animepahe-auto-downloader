...
def stub_state_store(monkeypatch: pytest.MonkeyPatch) -> list[dict]:
    ...
    monkeypatch.setattr(state_store, "upsert_download_task", lambda task: records.append(dict(task)))
    monkeypatch.setattr(state_store, "upsert_download_tasks", lambda tasks: records.extend(dict(task) for task in tasks))
...
async def test_unresolved_task_uses_resolver_and_updates_resolution(
    ...
    task = await manager.add_task(
        ...
        download_options=[{"pahe_link": "https://pahe.win/e1", "quality": "720p", "resolution": 720}],
    )
...
def test_link_expired_failure_can_retry_with_fresh_resolve(
    ...
    manager = DownloadManager(str(tmp_path), max_workers=1)
    manager.set_link_resolver(lambda task: "https://cdn.example/e1.mp4")
    ...
    assert manager._should_retry_with_fresh_link(task, RuntimeError("Kwik returned HTTP 403"))
    assert manager._should_retry_with_fresh_link(task, RuntimeError("Failed to fetch https://pahe.win/e after 3 attempts: HTTP 429"))
    assert not manager._should_retry_with_fresh_link(task, RuntimeError("Kwik still returned HTTP 403"))
    task.retry_count = 1
    assert not manager._should_retry_with_fresh_link(task, RuntimeError("Kwik returned HTTP 403"))
